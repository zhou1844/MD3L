...
val AccentSunsetGlow = AccentPalette(
    ...
    secondary = Color(0xFFD6A9A0),
    onSecondary = Color(0xFF3B2420),
    secondaryContainer = Color(0xFF543A35),
    onSecondaryContainer = Color(0xFFF5C6BD),
    tertiary = Color(0xFFC0B0E8),
    onTertiary = Color(0xFF2C2450),
    tertiaryContainer = Color(0xFF433768),
    onTertiaryContainer = Color(0xFFDCD8FF),
...
val AccentOceanMint = AccentPalette(
    ...
    primaryContainer = Color(0xFF005754),
    onPrimaryContainer = Color(0xFF9CF2EE),
    secondary = Color(0xFFA8C8C4),
    onSecondary = Color(0xFF1A3030),
    secondaryContainer = Color(0xFF304844),
    onSecondaryContainer = Color(0xFFC4E8E4),
    tertiary = Color(0xFF9AE4B8),
    onTertiary = Color(0xFF003824),
    tertiaryContainer = Color(0xFF005238),
    onTertiaryContainer = Color(0xFFB8F4D4),
...
val AccentFireIce = AccentPalette(
    ...
    primaryContainer = Color(0xFF7E262E),
    onPrimaryContainer = Color(0xFFFFDADC),
    secondary = Color(0xFFB8C8E8),
    onSecondary = Color(0xFF1E2A44),
    secondaryContainer = Color(0xFF35405C),
    onSecondaryContainer = Color(0xFFD4E0FC),
    tertiary = Color(0xFFB0B8E8),
    onTertiary = Color(0xFF242854),
    tertiaryContainer = Color(0xFF3C3E6C),
    onTertiaryContainer = Color(0xFFDCDEFF),
    inversePrimary = Color(0xFFA82028),
)
...
fun MD3LTheme(content: @Composable () -> Unit) {
    ...
    ThemeState.isDark = effectiveDark
    val baseScheme = if (effectiveDark) buildDarkScheme(ThemeState.accent) else buildLightScheme(ThemeState.accent)
...
@Composable
private fun rememberSystemDark(): Boolean {
    val isLinux = System.getProperty("os.name").lowercase().contains("linux")
    if (!isLinux) return isSystemInDarkTheme()
    var dark by remember { mutableStateOf(detectLinuxSystemDark()) }
    LaunchedEffect(Unit) {
        while (true) {
            dark = detectLinuxSystemDark()
            delay(3000)
        }
    }
    return dark
...
private fun detectLinuxSystemDark(): Boolean {
    val gtkTheme = System.getenv("GTK_THEME") ?: ""
    if (gtkTheme.lowercase().contains("dark")) return true
    val settingsIni = File(System.getProperty("user.home"), ".config/gtk-3.0/settings.ini")
    if (settingsIni.exists()) {
        runCatching {
            val text = settingsIni.readText(Charsets.UTF_8)
            if (text.contains("gtk-application-prefer-dark-theme=1")) return true
            if (text.contains("gtk-theme-name=") && text.lowercase().contains("dark")) return true
        }
    }
    val gsettingsOut = runCatching {
        val proc = ProcessBuilder("gsettings", "get", "org.gnome.desktop.interface", "color-scheme")
            .redirectErrorStream(true).start()
        val out = proc.inputStream.bufferedReader().readText().trim()
        proc.waitFor(2, TimeUnit.SECONDS)
        out
    }.getOrNull() ?: ""
    if (gsettingsOut.contains("prefer-dark") || gsettingsOut.contains("'dark'")) return true
    return false
}
