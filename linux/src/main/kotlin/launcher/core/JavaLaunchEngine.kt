...
class JavaLaunchEngine : ILaunchEngine {
     ...
     *
     * 完全不用资源包方式——资源包方案在高版本 MC（1.21.11+、快照版 26w06a 等）存在
    ...
    private fun buildJvmArguments(
        ...
        if (needsAuthlibInjector) {
            ...
            if (!authlibInjectorPath.exists() || authlibInjectorPath.length() == 0L) {
                ...
                try {
                    val proc = ProcessBuilder(LauncherDirs.curlCmd(), "-sL", url).start()
                    ...
                    val downloadProc = ProcessBuilder(
                        LauncherDirs.curlCmd(), "-sL", "-o", authlibInjectorPath.absolutePath, downloadUrl
                    ).start()
            ...
            if (authlibInjectorPath.exists() && authlibInjectorPath.length() > 0L) {
                ...
                // OfflineAuthInfo.getLaunchArguments() 也添加此属性
    ...
    private fun curlGet(url: String): String? {
        return runCatching {
            val proc = ProcessBuilder(LauncherDirs.curlCmd(), "-sL", "--connect-timeout", "10", "--max-time", "30", url)
