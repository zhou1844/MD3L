...
object ModpackManager {
        ...
        try {
            try {
                ...
                if (definition.kind == PackKind.Modrinth && definition.modrinthIndex != null) {
                    ...
                    val dlThreads = settings.maxDownloadThreads.coerceIn(16, 128)
                    downloadModrinthFiles(definition.modrinthIndex, instanceDir, logger, dlThreads) { step, frac ->
                        val mapped = (0.70f + 0.24f * frac).coerceIn(0.70f, 0.94f)
                        report(step, mapped)
                    }
                }

                if (definition.kind == PackKind.CurseForge && definition.curseFiles.isNotEmpty()) {
                    println("[ModpackImport] 步骤5: 下载 CurseForge 模组文件...")
                    val dlThreads = settings.maxDownloadThreads.coerceIn(16, 128)
                    downloadCurseForgeFiles(definition.curseFiles, instanceDir, logger, dlThreads) { step, frac ->
                        val mapped = (0.70f + 0.24f * frac).coerceIn(0.70f, 0.94f)
                        report(step, mapped)
                    }
                }
        ...
        if (manifestPair != null) {
            ...
            if (manifest != null && manifest["minecraft"] is JsonObject) {
                ...
                val curseFiles = manifest["files"]?.jsonArray?.mapNotNull { el ->
                    ...
                    val fileID = obj["fileID"]?.jsonPrimitive?.intOrNull ?: return@mapNotNull null
                    val required = obj["required"]?.jsonPrimitive?.booleanOrNull ?: true
                    CurseFileRef(projectID = projectID, fileID = fileID, required = required)
                } ?: emptyList()
                logger.info("CurseForge manifest 包含 ${curseFiles.size} 个文件引用")
                println("[ModpackImport] CurseForge manifest files: ${curseFiles.size} 个")
                return PackDefinition(
    ...
    private suspend fun installLoaderIfNeeded(
        ...
        if (existingId != null) {
            ...
            onLoaderProgress("检测到已安装 $loaderType $loaderVersion，复用中", 0.2f)
            ...
            if (ready) {
                onLoaderProgress("已复用 $loaderType $loaderVersion（无需重新安装）", 1.0f)
                kotlinx.coroutines.delay(400)
    ...
    private suspend fun downloadModrinthFiles(
        ...
        logger.info("并发下载 $total 个资源")
        onProgress("并发下载整合包资源 0/$total", 0f)
        try {
            DownloadManager.downloadAllIsolated(tasks, maxConcurrency = 16) { completed, _, currentFile ->
    ...
    private suspend fun downloadCurseForgeFiles(
        ...
        if (curseFiles.isEmpty()) {
            onProgress("CurseForge 模组已就绪", 0.94f)
            return@withContext
        }
        ...
        val total = curseFiles.size
        logger.info("查询 $total 个 CurseForge 文件信息（逐文件 API，参照 HMCL CurseCompletionTask）")
        println("[ModpackImport] CurseForge: 逐文件查询 $total 个文件信息...")
        onProgress("查询 CurseForge 文件信息 0/$total", 0f)

        val sem = Semaphore(minOf(16, maxThreads.coerceIn(4, 64)))
        val finished = java.util.concurrent.atomic.AtomicInteger(0)
        val failed = java.util.concurrent.atomic.AtomicInteger(0)

        data class ResolvedCfFile(
            val ref: CurseFileRef,
            val downloadUrl: String,
            val fileName: String,
            val fileSize: Long,
        )

        val resolved = java.util.Collections.synchronizedList(mutableListOf<ResolvedCfFile>())

        coroutineScope {
            curseFiles.map { ref ->
                launch(Dispatchers.IO) {
                    sem.withPermit {
                        var cfFile: CurseForgeFile? = null
                        try {
                            cfFile = ModrinthApi.getCurseForgeFile(ref.projectID, ref.fileID)
                        } catch (_: Exception) {}

                        if (cfFile != null && cfFile.fileName.isNotBlank()) {
                            resolved.add(ResolvedCfFile(
                                ref = ref,
                            ...
                            logger.warn("CurseForge 文件查询失败: projectID=${ref.projectID}, fileID=${ref.fileID}")
                        }

                        val done = finished.incrementAndGet()
                        if (done % 10 == 0 || done == total) {
                            onProgress("查询 CurseForge 文件信息 $done/$total", done.toFloat() / total * 0.30f)
                        }
                    }
                }
            }.joinAll()
        ...
        val tasks = resolved.mapNotNull { r ->
            ...
            if (effectiveFileName.isBlank()) return@mapNotNull null

            val dest = File(modsDir, effectiveFileName)
            if (dest.isFile && dest.length() > 0 && (r.fileSize <= 0 || dest.length() == r.fileSize)) {
                return@mapNotNull null
            }

            val candidates = buildCurseForgeCandidateUrls(r.downloadUrl, r.ref.projectID, r.ref.fileID, effectiveFileName)
            DownloadTask(
        ...
        if (tasks.isEmpty()) {
            logger.info("全部 CurseForge 模组已存在或无法获取下载链接")
            onProgress("CurseForge 模组已就绪", 0.94f)
            return@withContext
        }
        ...
        logger.info("并发下载 $taskCount 个 CurseForge 模组文件")
        println("[ModpackImport] CurseForge: 开始下载 $taskCount 个模组文件...")
        onProgress("下载 CurseForge 模组 0/$taskCount", 0.30f)

        try {
            DownloadManager.downloadAllIsolated(tasks, maxConcurrency = maxThreads.coerceIn(4, 64)) { _, _, currentFile ->
                val done = downloaded.incrementAndGet()
                val fraction = 0.30f + 0.64f * (done.toFloat() / taskCount)
                onProgress("下载 CurseForge 模组 $done/$taskCount: $currentFile", fraction)
            }
            logger.info("CurseForge 模组下载完成: $taskCount 个")
            println("[ModpackImport] CurseForge: 下载完成 $taskCount 个模组")
            onProgress("CurseForge 模组下载完成", 0.94f)
        } catch (e: Exception) {
            logger.error("CurseForge 模组批量下载异常: ${e.message}")
            println("[ModpackImport] CurseForge 下载异常: ${e.message}")
            onProgress("CurseForge 模组下载部分完成", 0.94f)
        }
    }
