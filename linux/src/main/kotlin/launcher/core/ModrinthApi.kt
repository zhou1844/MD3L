...
object ModrinthApi {
    ...
    suspend fun getCurseForgeFile(modId: Int, fileId: Int): CurseForgeFile? = withContext(Dispatchers.IO) {
        try {
            val resp = client.get("$CF_PROXY/mods/$modId/files/$fileId") {
                header("Accept", "application/json")
                header("User-Agent", "MD3L/1.1")
            }
            ...
            CurseForgeFile(
                fileId = data["id"]?.jsonPrimitive?.intOrNull ?: fileId,
                fileName = data["fileName"]?.jsonPrimitive?.contentOrNull ?: "",
                displayName = data["displayName"]?.jsonPrimitive?.contentOrNull ?: "",
                downloadUrl = data["downloadUrl"]?.jsonPrimitive?.contentOrNull ?: "",
                fileSize = data["fileLength"]?.jsonPrimitive?.longOrNull ?: 0L,
                gameVersions = data["gameVersions"]?.jsonArray?.mapNotNull { it.jsonPrimitive.contentOrNull } ?: emptyList(),
                releaseDate = data["fileDate"]?.jsonPrimitive?.contentOrNull ?: "",
            )
        } catch (_: Exception) {
            null
        }
